...
// interface which is backed by an active websockets connection to utreexod.
type UtreexodFilteredChainView struct {
	...
	utreexodConn *chain.UtreexoRPCClient
...
// from RPC credentials for an active utreexod instance.
func NewUtreexodFilteredChainView(config *chain.UtreexodRPCClientConfig,
	blockCache *blockcache.BlockCache) (*UtreexodFilteredChainView, error) {

	chainView := &UtreexodFilteredChainView{
	...
	config.Conn.DisableConnectOnNew = true
	config.Conn.DisableAutoReconnect = false

	config.NotificationHandlers = ntfnCallbacks

	chainConn, err := chain.NewUtreexodRPCClientWithConfig(config)
	if err != nil {
		return nil, err
	}
	chainView.utreexodConn = chainConn
...
func (b *UtreexodFilteredChainView) Start() error {
	// Already started?
	...
	// disconnected blocks.
	if err := b.utreexodConn.Connect(20); err != nil {
		return err
	}
	if err := b.utreexodConn.NotifyBlocks(); err != nil {
		return err
	}

	_, bestHeight, err := b.utreexodConn.GetBestBlock()
	if err != nil {
		return err
	}
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *UtreexodFilteredChainView) Stop() error {
...
func (b *UtreexodFilteredChainView) onFilteredBlockConnected(height int32,
	uHeader *utreexowire.BlockHeader, uTxns []*utreexobtcutil.Tx) {

	mtxs := make([]*wire.MsgTx, len(uTxns))
	b.filterMtx.Lock()
	for i, uTx := range uTxns {
		...
		if err := umtx.Serialize(buf); err != nil {
			log.Errorf("unable to serialize tx: %v", err)
			continue
		}

		var mtx *wire.MsgTx
		if err := mtx.Deserialize(buf); err != nil {
	...
	block := &FilteredBlock{
		Hash:         chainhash.Hash(uHeader.BlockHash()),
...
func (b *UtreexodFilteredChainView) onFilteredBlockDisconnected(height int32,
	header *utreexowire.BlockHeader) {

	log.Debugf("got disconnected block at height %d: %v", height,
		header.BlockHash())

	filteredBlock := &FilteredBlock{
		Hash:   chainhash.Hash(header.BlockHash()),
		Height: uint32(height),
	}
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *UtreexodFilteredChainView) FilterBlock(blockHash *chainhash.Hash) (*FilteredBlock, error) {
	req := &filterBlockReq{
		blockHash: blockHash,
...
func (b *UtreexodFilteredChainView) chainFilterer() {
	defer b.wg.Done()
	...
	for {
		...
		case update := <-b.filterUpdates:
			...
			b.utreexodConn.LoadTxFilter(false, []btcutil.Address{},
				update.newUtxos)
			...
			for i := update.updateHeight + 1; i < bestHeight+1; i++ {
				blockHash, err := b.utreexodConn.GetBlockHash(int64(i))
				if err != nil {
				...
				// have gone missing.
				rescanned, err := b.utreexodConn.RescanBlocks(
		...
		case req := <-b.filterBlockReqs:
			...
			header, err := b.utreexodConn.GetBlockHeaderVerbose(req.blockHash)
			if err != nil {
				req.err <- err
				req.resp <- nil
				continue
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *UtreexodFilteredChainView) UpdateFilter(ops []channeldb.EdgePoint,
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *UtreexodFilteredChainView) FilteredBlocks() <-chan *FilteredBlock {
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *UtreexodFilteredChainView) DisconnectedBlocks() <-chan *FilteredBlock {
	return b.blockQueue.staleBlocks
}

// GetBlock is used to retrieve the block with the given hash. This function
// wraps the blockCache's GetBlock function.
func (b *UtreexodFilteredChainView) GetBlock(hash *chainhash.Hash) (
	*wire.MsgBlock, error) {

	return b.blockCache.GetBlock(hash, b.utreexodConn.GetBlock)
}
