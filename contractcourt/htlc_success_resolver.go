...
// ErrSecondLevelTxNotSwept is returned when attempting to sweep the output of
// a second level tx without it being spent.
var ErrSecondLevelTxNotSwept = errors.New("second level tx not swept")
...
func (h *htlcSuccessResolver) Resolve() (ContractResolver, error) {
	...
	case h.isZeroFeeOutput():
		err = h.resolveReSignedSuccessTx()
...
// resolveLegacySuccessTx handles an HTLC output from a pre-anchor type
// channel by broadcasting the second-level success transaction. It returns the
// ultimate outpoint of the second-level tx, that we must wait to be spent for
// the resolver to be fully resolved.
...
// resolveReSignedSuccessTx waits for the sweeping tx of the second-level
// success tx to confirm and offers the output from the success tx to the
// sweeper.
func (h *htlcSuccessResolver) resolveReSignedSuccessTx() error {
	...
	if !h.outputIncubating {
		...
		h.log.Infof("second-level HTLC success transaction confirmed!")
	}
	...
	// We now check whether the sweep request for the output on the
	// second-level tx has already been made.
	h.reportLock.Lock()
	successTxOutputSwept := h.currentReport.Stage == 2
	h.reportLock.Unlock()

	// Send it if the sweep request hasn't been made.
	if !successTxOutputSwept {
