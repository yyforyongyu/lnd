...
func (h *htlcTimeoutResolver) claimCleanUp(
	...
	h.log.Infof("HTLC(%x) has been swept with pre-image by remote party "+
		"during timeout flow! Adding pre-image to witness cache",
		h.htlc.RHash[:])
	...
	if err != nil {
		return fmt.Errorf("unable to create pre-image from "+
			"witness: %v", err)
	}
...
func (h *htlcTimeoutResolver) Resolve() (ContractResolver, error) {
	...
	return nil, h.resolveLocalCommitOutput()
}
...
func (h *htlcTimeoutResolver) sweepTimeoutTx() error {
	...
	h.log.Infof("offering 2nd-level HTLC timeout tx to sweeper with "+
		"deadline=%v, budget=%v", h.incomingHTLCExpiryHeight, budget)
...
func (h *htlcTimeoutResolver) watchHtlcSpend() (*chainntnfs.SpendDetail,
	...
	h.log.Debugf("waiting for spent of HTLC output %v to be confirmed",
		outpointToWatch)
	...
	for {
		...
		case spendDetail, ok := <-blockSpent:
			if !ok {
				...
				if !hasMempoolSpent {
					result.err = h.checkpointStageOne(
						*spendDetail.SpenderTxHash,
					)
				}
			}
...
func (h *htlcTimeoutResolver) Launch() error {
	...
	default:
		...
		// Otherwise, sweep the second level timeout tx.
...
func (h *htlcTimeoutResolver) resolveTimeoutTxOutput(op wire.OutPoint) error {
	h.log.Debugf("waiting for 2nd-level timeout tx output %v to be "+
		"spent after csv_delay=%v", op, h.htlcResolution.CsvDelay)
...
// resolveLocalCommitOutput waits for the sweeping tx of the second-level
// timeout tx to confirm and offers the output from the timeout tx to the
// sweeper.
func (h *htlcTimeoutResolver) resolveLocalCommitOutput() error {
	h.log.Debug("waiting for 2nd-level HTLC timeout tx to confirm")
	...
	// Start the process to sweep the output from the re-signed timeout tx.
	if h.isZeroFeeOutput() {
		err = h.sweepTimeoutTxOutput()
	} else {
		// Start the process to sweep the output from the legacy
		// timeout tx.
		err = h.sendSecondLevelTxLegacy()
	}
...
func (h *htlcTimeoutResolver) resolveRemoteCommitOutput() error {
	...
	h.log.Tracef("htlc output spent by tx=%v", sweepTxDetails.SpenderTxHash)
	...
	if isPreimageSpend(
		...
		return h.claimCleanUp(sweepTxDetails)
	}
	...
	return h.checkpointClaim(sweepTxDetails)
}
