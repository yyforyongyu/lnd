...
var multiHopForceCloseTestCases = []*lntest.TestCase{
	{
		Name:     "multi hop local timeout anchor",
		TestFunc: testMultiHopLocalTimeoutAnchor,
	},
	{
		Name:     "multi hop local timeout leased",
		TestFunc: testMultiHopLocalTimeoutLeased,
	},
	{
		Name:     "multi hop local timeout simple taproot",
		TestFunc: testMultiHopLocalTimeoutSimpleTaproot,
	},
	{
		Name:     "multi hop receiver claim anchor",
		TestFunc: testMultiHopReceiverClaimAnchor,
	},
	{
		Name:     "multi hop receiver claim leased",
		TestFunc: testMultiHopReceiverClaimLeased,
	},
	{
		Name:     "multi hop receiver claim simple taproot",
		TestFunc: testMultiHopReceiverClaimSimpleTaproot,
	},
}

// testMultiHopLocalTimeoutAnchor tests `runMultiHopLocalTimeout` with anchor
// channel.
func testMultiHopLocalTimeoutAnchor(ht *lntest.HarnessTest) {
	success := ht.Run("no zero conf", func(t *testing.T) {
		...
		runMultiHopLocalTimeout(st, cfgs, openChannelParams)
	})
	if !success {
		return
	}
...
// testMultiHopLocalTimeoutLeased tests `runMultiHopLocalTimeout` with script
// enforced lease channel.
func testMultiHopLocalTimeoutLeased(ht *lntest.HarnessTest) {
	success := ht.Run("no zero conf", func(t *testing.T) {
		st := ht.Subtest(t)
...
// testMultiHopLocalTimeoutSimpleTaproot tests `runMultiHopLocalTimeout` with
// simple taproot channel.
func testMultiHopLocalTimeoutSimpleTaproot(ht *lntest.HarnessTest) {
...
// runMultiHopLocalTimeout tests that in a multi-hop scenario, if the outgoing
// HTLC is about to time out, then we'll go to chain in order to claim it using
// the HTLC timeout transaction. Any dust HTLC's should be immediately canceled
// backwards. Once the timeout has been reached, then we should sweep it
// on-chain, and cancel the HTLC backwards.
func runMultiHopLocalTimeout(ht *lntest.HarnessTest,
...
// testMultiHopReceiverClaimAnchor tests `runMultiHopReceiverClaim` with anchor
// channels.
func testMultiHopReceiverClaimAnchor(ht *lntest.HarnessTest) {
	success := ht.Run("no zero conf", func(t *testing.T) {
		st := ht.Subtest(t)
		...
		runMultiHopReceiverClaim(st, cfgs, openChannelParams)
	})
	if !success {
		return
	}
...
// testMultiHopReceiverClaimLeased tests `runMultiHopReceiverClaim` with script
// enforce lease channels.
func testMultiHopReceiverClaimLeased(ht *lntest.HarnessTest) {
	success := ht.Run("no zero conf", func(t *testing.T) {
		st := ht.Subtest(t)
...
// testMultiHopReceiverClaimSimpleTaproot tests `runMultiHopReceiverClaim` with
// simple taproot channels.
func testMultiHopReceiverClaimSimpleTaproot(ht *lntest.HarnessTest) {
	c := lnrpc.CommitmentType_SIMPLE_TAPROOT
...
func runMultiHopReceiverClaim(ht *lntest.HarnessTest,
	cfgs [][]string, params lntest.OpenChannelParams) {
